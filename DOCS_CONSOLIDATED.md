...
<a id="46"></a>
## ٤٦. نموذج المصدر المستهدف — الطور الأول
...
<a id="47"></a>
## ٤٧. الحسابات والوصول والإسناد — الطور الثاني
...
## ٤٨. زمن التشغيل للجمع — الطور الثالث
...
1. **عدم التراجع** — لا تعود إلى الوراء أبدًا. مفروضة بمُشغِّل قاعدة
   بيانات (ترحيل 0029)، لا بحارس بايثون وحده: قاعدة يعرفها التطبيق وحده
...
## ٤٩. دليل النشر — من فرع مُثبَت إلى إنتاج يعمل
...
فرع العمل يحمل **أربعة ترحيلات** ليست على `main`: `0026` و`0027` و`0028`
و`0029`. وقالب Render يقرأ `branch: main`، وأمر بدئه هو
`alembic upgrade head && uvicorn …`. أي أنّ **الدمج إلى `main` هو نفسه
حدث ترحيل على قاعدة الإنتاج**، يجري تلقائيًّا عند أوّل نشر، وقبل أن تخدم
الخدمة أيّ طلب.

هذه ليست تفصيلة نشر. إنّها الخطوة الأخطر في العمليّة كلّها، ولها ملاحظتان:

- `0026` يملأ `resources` و`occurrences` من `links`. قِيس محليًّا:
  **23.1 ثانية** لـ200 ألف مورد و600 ألف ظهور. إنتاج أكبر يعني وقتًا
  ...
  عطل.
- `0027` يركّب حارس مرآة الإسناد، و`0029` يركّب حارسَي عدم التراجع. بعدها
...
`.github/workflows/backup.yml` يأخذ `pg_dump` مشفّرًا يوميًّا الساعة
03:00 UTC ويحفظه 30 يومًا. **شغّله يدويًّا قبل النشر** ولا تعتمد على
...
`0029` يُلزِم نفسه بأن يكون عدد صفوف `source_progress` مساويًا لعدد
