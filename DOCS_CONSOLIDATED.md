...
## ٥٢. البحث والاستيراد والتصدير — الطور الخامس

> **لا هجرة، ولا عمود جديد، ولا كاتب جديد.** الطور كلّه فوق `links`،
> النموذج التشغيلي الوحيد، وقرارُ ذلك مبنيّ على قياس لا على تفضيل.

### ٥٢.١ القرار المعماري: لماذا `links` ولماذا ليس `Resource/Occurrence`

السؤال الذي كان يجب حسمه قبل أيّ سطر: هل تحقيق معايير قبول الطور الخامس
يتطلّب أن تكون `Resource`/`Occurrence` بيانات تشغيلية حيّة؟

الدليل، مقيساً لا مستنتجاً:

| الحقيقة | الدليل | الدرجة |
|---|---|---|
| `links` هو السلطة التشغيلية الوحيدة | `Link(` يُنشأ في موضعين فقط في المستودع: `app/ingest.py:192` و`app/routers/links.py:868`. صفر إنشاءات لـ`Resource(`/`Occurrence(` خارج `app/models.py` وملف اختبار واحد | (أ) |
| `resources` و`occurrences` فارغتان في الإنتاج | `resources_total=0`، `occurrences_total=0` مقابل `links_total=2155` — تشغيل `33965647501`، مقيس بـ`app.rls.for_each_workspace` لأن الجدولين تحت FORCE RLS ولولا ذلك لقرأ صفراً مهما كان فيهما | (أ) |
| ذلك ليس حادثاً | `links_before_2026-09-03T17:40:31 = 0` و`links_min_created_at = 2026-09-04 17:17:40` — لم يوجد رابط واحد قبل تشغيل الهجرة 0028، فنسخ الـbackfill الصفرَ الذي وجده | (أ) |
| تفعيلهما **لا يفتح** «البحث بنوع الرابط» | `Resource.link_type` معرَّف `NULL` لكل صفّ مُرحَّل، وتعليقه في `app/models.py` يقول صراحةً: «يملؤه الطور الذي يبني تصنيف الروابط». backfill اليوم ينتج ٢١٥٥ صفّاً كلّها `link_type = NULL` | (أ) |

البند الأخير هو الحاسم. الحجّة الوحيدة المعقولة لتفعيل النموذج المستهدف
داخل هذا الطور كانت أنّه يفتح معياراً من معايير القبول. لا يفتحه. نوع
الرابط محجوب على **طور التصنيف**، لا على امتلاء `Resource`.

فالنتيجة: **NOT BLOCKING** بمعنى §٧ — لا معيار قبول في الطور الخامس
يتعذّر بدونهما، ولا يخلق البناء فوق `links` سلطةً مزدوجة، لأن `links` هي
السلطة الوحيدة أصلاً ولم يُضَف كاتب ثانٍ.

وما يُمنع صراحةً: تسمية استعلامٍ فوق `links` باسم «Resource Search». ذلك
تزوير دلالي. `Resource`/`Occurrence` حالتهما **DEFERRED** ومكتوبة كذلك،
لا PASS ولا UNVERIFIED.

### ٥٢.٢ العيب الذي كان يفقد الروابط بصمت

كل ترتيب في المنتج كان ينتهي على عمود غير فريد — `created_at` في خمسة
منها، وهو `default=utcnow` يُقيَّم لكل صفّ عند الـflush، فدفعة كُتبت في
معاملة واحدة تتساوى إلى الميكروثانية. و`OFFSET`/`LIMIT` يُرقّم بالموضع
داخل ترتيبٍ تتركه SQL غير معرَّف عند التساوي.

مقيس على PostgreSQL 16، ٢٠٠٠ رابط تتقاسم `created_at` واحدة، بصفحات من
خمسة:

```
ORDER BY created_at DESC              1802 من 2000 صفّ يمكن بلوغه
ORDER BY created_at DESC, id DESC     2000 من 2000
```

١٩٨ رابطاً — ٩٫٩٪ — لا يمكن رؤيتها عبر الواجهة إطلاقاً، بينما `total`
يعدّها، وصفوف أخرى تُقدَّم مرّتين مكانها، بلا خطأ ولا إشارة.

**وهو معتمد على الخطّة**، ولذلك صار الـtie-breaker غير مشروط: القياس نفسه
على عمود له فهرس صالح يبلغ ٢٠٠٠/٢٠٠٠ بلا tie-breaker، لأن المخطّط يجيب من
`Index Scan Backward` بترتيب واحد ثابت بدل heapsort. أي أن `sort=date`
سليم اليوم **فقط** لوجود `ix_links_workspace_created`، و`sort=domain`
يفقد الـ١٩٨. إضافة فهرس أو حذفه أو نموّ جدول يحرّك الترتيب بين الخطّتين بلا
إعلان.

### ٥٢.٣ التصدير كان يوسّع نطاقه بنفسه

البحث كان يقبل عشرة مرشّحات؛ التصديرات الثلاثة تقبل أربعة منها، وتتجاهل
الستّة الباقية، وتفرض `include_archived=True` فوق ذلك. فمن صفّى إلى
مفضّلاته وصدّر، حصل على المساحة كاملة — كلّ تصنيف، وكلّ نطاق، وكلّ رابط
أرشفه عمداً — بـ200 ولا شيء في الاستجابة يقول إنّ مرشّحاً أُسقط.

اتجاه العطل هو ما يجعله خطيراً: التصدير الذي يتجاهل مرشّحاً يعيد **أكثر**
ممّا طُلب، فالعَرَض هو خروج بيانات ظنّ صاحبها أنّه استبعدها.

العلاج بنيوي لا بتحرير أربعة توقيعات لتتّفق — فاتّفاقها اليدوي هو ما
أنتج هذا: `link_filters` يعلن مجموعة المرشّحات مرّة، والبحث والتصديرات
الثلاثة تستهلك `LinkFilters` نفسه، و`scope()` يعيد الاستعلام **مصفّى
ومرتّباً** معاً.

تغيير سلوك واحد مقصود: `/links/export.{csv,json,md}` صارت تحترم
`include_archived` كالبحث، افتراضه `false`. حجّة «الاكتمال» التي كان
التعليق القديم يسوقها حقيقية، وتخدمها نقطة أخرى بالفعل — `/auth/me/export`
يعيد المساحة كاملة بأرشيفها، وهو مسار النقل الموثَّق. والتصدير المصفّى
معناه «صدّر هذا البحث».

### ٥٢.٤ الاستيراد الجماعي — نقطة واحدة براية

`POST /channels/import` تأخذ قائمة ملصقة وراية `commit`:

```
commit=false (الافتراضي)  →  يحلّل ويتحقّق ويكشف التكرار، ولا يكتب شيئاً
commit=true               →  ينفّذ نفس شيفرة التخطيط، ثم ينشئ ما وسمه new
```

نقطة واحدة براية لا نقطتان، لأن المعاينة لا تساوي إلا ما يفي به التنفيذ:
محاكاة تجري شيفرة غير شيفرة المسار الحقيقي وعدٌ قابل لأن يخالف ما يتنبّأ
به. و«الأسلم» هو النداء الأقصر: استيراد يكتب ما لم يُمنع له تشغيل جافّ لا
يستعمله أحد.

قرارات مُعلَنة لا ضمنية:

- **دلالات المعاملة (§١٨): validated partial commit.** سطر واحد غير مقروء
  في قائمة من مئتين لا يجوز أن يردّ التسعة والتسعين بعد المئة. فكل سطر
  يعود بحالته الخاصّة — `new` / `duplicate` / `repeated` / `invalid` /
  `needs_account` — وبسببها. كلّها، لا الإخفاقات وحدها: «١٧ أُضيفت» يترك
  من ألصق القائمة عاجزاً عن معرفة أيّ ثلاثة سقطت.
- **رابط الدعوة أو المعرّف الرقمي = `needs_account` لا `invalid`.** كلاهما
  مرجع يفهمه النظام ولا يُقرأ بلا حساب منضمّ. تسجيله ينتج مصدراً لا يجمع
  شيئاً، وهو ما يشبه تماماً قناةً لا تنشر شيئاً — التمويه الذي يمنعه
  AC-A04.
- **التراجع يرفض، لكل مصدر، حذف مصدرٍ جمع روابط** منذ الاستيراد، ويبلّغ
  عنه في `kept`. المصدر الذي جمع لم يعد أثر الاستيراد وحده، وحذفه يأخذ
  الروابط معه — وهو ما يمنعه AC-I06 وAC-S04.
- **المنشأ في سجلّ التدقيق لا في عمود.** `channels` بلا عمود منشأ،
  وإضافته هجرة على قاعدة حيّة لا يأخذها هذا الطور. صفّ التدقيق يحمل
  معرّفات ما أنشأه الدُفعة، وهو ما يجعل التراجع قابلاً للتتبّع وما يميّز
  مصدراً مستورَداً عن `channel.add` أو `channel.add_public`. **والقيد
  حقيقي**: «أيّ المصادر جاءت باستيراد؟» يُجاب بقراءة سجلّ التدقيق لا
  بترشيح عمود.

مقبض التراجع هو معرّف صفّ التدقيق نفسه، فالمخوِّل للتراجع وسجلّ ما
سيتراجع عنه شيء واحد لا شيئان يستطيعان الاختلاف.

### ٥٢.٥ القياسات

PostgreSQL 16، ٢٠٠٬٠٠٠ رابط، ٢٠٠ رابط لكل طابع زمني (كثافة تساوٍ متشائمة
عمداً)، بالميلي ثانية:

| الاستعلام | p50 | p95 | p99 |
|---|---|---|---|
| صفحة ١ بلا tie-breaker | ٠٫١٥ | ٠٫٢٢ | ٠٫٤٨ |
| صفحة ١ بـtie-breaker | ٠٫٣٥ | ٠٫٤٤ | ٠٫٧٥ |
| صفحة ١٠٠ بلا / بـ | ٠٫٧٧ / ٢٫٣٩ | ٠٫٩٠ / ٢٫٨٣ | ١٫٠٠ / ٤٫٨٤ |
| صفحة ١٠٠٠ بلا / بـ | ٦٫٤٩ / ٢٢٫٢٢ | ٦٫٩٣ / ٢٩٫٠٨ | ٧٫٣٩ / ٣٦٫٥٣ |
| بحث نصّي كامل | ٣٫٦٤ | ٤٫٢٢ | ٥٫٣٤ |
| منصّة + تصنيف | ٠٫٣٣ | ٠٫٥٣ | ٠٫٥٤ |
| ترتيب بالنطاق | ٤٫١٧ | ٤٫٨٠ | ٥٫٧٥ |
| `count(*)` لحقل `total` | ٢٠٫٧٨ | ٢١٫٥٧ | ٢٢٫٢٣ |

**ثمن الـtie-breaker مقيس ومقبول، ولا فهرس جديد.** المخطّط يختار
`Incremental Sort`: يبقي مسح الفهرس (مرتَّب مسبقاً على `created_at`)
ويرتّب داخل كل مجموعة متساوية فقط. الثمن ٣× على الترقيم العميق، و٢٢ms عند
الصفحة ١٠٠٠ من ٢٠٠ ألف صفّ ليس مشكلة مقيسة — والبديل فهرس
`(workspace_id, created_at DESC, id DESC)` يكلّف على كل إدراج ويحتاج هجرة
على قاعدة حيّة. الشرط في §٢٣ صريح: لا فهرس بلا مشكلة مقيسة. لا توجد
مشكلة. الإنتاج فيه ٢١٥٥ رابطاً، والصفحة ١٠٠٠ لا وجود لها فيه.

ملاحظة جانبية مقيسة ولم تُعالَج لأنها سابقة لهذا الطور: `count(*)` لحقل
`total` أغلى من جلب الصفحة نفسها عند أيّ عمق دون ١٠٠٠.

والاستيراد، بعد إصلاح N+1 وُجد أثناء القياس نفسه:

```
معاينة (مساحة فارغة، ٢٠٠ سطر)          ٤٢٫١ ms
تنفيذ  (ينشئ ٢٠٠)                       ٩١٫٩ ms
إعادة تنفيذ (٢٠٠ مكرّرة)   ٦٢٢  →       ١٥٫٧ ms
معاينة (٢٠٠ سطر مقابل ٢٠٠ مصدر)         ١٣٫١ ms
```

شكل الأرقام هو ما كشف العيب: المسار الذي لا يكتب شيئاً كان أبطأ أربع
مرّات ونصفاً من المسار الذي يكتب كل شيء، لأن فحص التكرار كان يقرأ جدول
المصادر مرّة لكل سطر.

### ٥٢.٦ ما لم يُنجَز، ولماذا

| البند | الحالة | السبب |
|---|---|---|
| البحث بنوع الرابط | **DEFERRED** | `link_type` غير موجود كبيانات في أيّ مكان. على `Resource` معرَّف `NULL` لكل صفّ بنصّ تعليقه، وعلى `Link` غير موجود. بناؤه تصنيفٌ لا بحث، ويحتاج عموداً وهجرةً على قاعدة حيّة — بوّابة تفويض §٢٦/§٢٧ |
| البحث بالحساب | **DEFERRED** | قابل للوصل عبر `Channel.account_id` بلا هجرة، لكنه مرشّح جديد لا إصلاح عطل. خارج الحدّ الأدنى الذي يقتضيه §٣٦ |
| بحث Resource / بحث Occurrence | **DEFERRED** | الجدولان فارغان ولا كاتب لهما. تسمية استعلامٍ فوق `links` بأيّ منهما تزوير دلالي (§٨) |
| نشر إلى الإنتاج | **لم يُنفَّذ** | §٢٧: تنفيذ الطور لا يعني نشراً. العمل على الفرع، وmain لم تُمَسّ |
