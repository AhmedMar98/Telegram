...
| API + UI endpoints | 35 (31 distinct paths) | `app.openapi()["paths"]`, count values |
| Alembic migrations | 8 | `ls alembic/versions \| wc -l` |
| Test suite | 278 passing + 12 Postgres-only = 290; all 239 run together when `DATABASE_URL` and `PG_TEST_DSN` are separate databases | `pytest -q` in a clean venv |
...
| Field encryption | Telegram session strings encrypted at rest with Fernet (`FIELD_ENCRYPTION_KEY`); manual rotation supported | `app/crypto.py`, `scripts/rotate_encryption_key.py` |
| Password policy | NIST SP 800-63B reviewed: 8-char min, 200 max, no composition rules, no forced expiry, common-password blocklist | `app/passwords.py` |
| Request limits | 1 MiB body cap enforced by middleware before parsing | `app/main.py` |
...
| CI | `ruff check` + `ruff format --check` + `mypy app scripts` + `bandit -r app scripts tests -lll -iii` + `alembic upgrade head && alembic check` + full pytest, plus an independent `postgres-search` job against a real Postgres 16 service container | `.github/workflows/ci.yml` |
...
3. `bandit -r app scripts tests -lll -iii` reports no issues.
